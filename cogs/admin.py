...
class Admin(commands.Cog):
    ...
    async def blacklist(ctx, user: dc.member=None):
        ...
        if is_admin:
            if user == None:
                ctx.send("Error: you must select a user to blacklist")
            elif not (user in guild.members):
                ctx.send("Error: that member doesn't exist in this server")
            else:
                ctx.send("Blacklisting user... (don't worry this is just a test)")
